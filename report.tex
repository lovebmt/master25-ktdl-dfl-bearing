...
\subsection{Ứng Dụng DFL Trong Hệ Thống IoT Thực Tế}
...
\subsubsection{Smart Home và Smart Grid}
...
\subsubsection{Industrial IoT (IIoT)}
...
\midrule
Exp 1: FedAvg (Balanced) & 0.003939 & 0.004064 \\
Exp 2: FedAvg (Imbalanced) & 0.003891 & 0.003787 \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Quan sát:} Phản trực giác, phân phối không cân bằng cho kết quả tốt hơn so với phân phối cân bằng:

\begin{itemize}
    \item Eval loss của Non-IID (0.003787) thấp hơn IID (0.004064) đáng kể (giảm 6.82\%)
    \item Training loss cũng thấp hơn một chút (0.003891 vs 0.003939, giảm 1.22\%)
...
\begin{itemize}
    ...
    \item 95th percentile (0.078370) nằm ở vị trí phù hợp để phân biệt normal vs anomaly
    \item Mean + 2$\sigma$ (0.083116) cao hơn một chút, nhưng 95th percentile được chọn vì ít false positives hơn
\end{itemize}

\textbf{Cumulative Distribution:}
\begin{itemize}
    \item 95\% samples có MSE dưới threshold 0.078370
...
\begin{itemize}
    \item Median MSE: 0.052025 (lower than threshold)
...
\begin{itemize}
    ...
    \item Mean MSE: 0.053260
    \item Std deviation: 0.014928
    \item Min MSE: 0.014653
    \item Max MSE: 0.188146
    \item Median: 0.052025
    \item Mean + 2$\sigma$: 0.083116
\end{itemize}
...
\midrule
Normal Sample & 0.062347 & < 0.078370 & \textcolor{green}{NORMAL} \\
Scenario 1: Sensor Error & 0.512309 & > 0.078370 & \textcolor{red}{ANOMALY} \\
Scenario 2: High Vibration & 0.215080 & > 0.078370 & \textcolor{red}{ANOMALY} \\
Scenario 3: Negative Values & 0.124484 & > 0.078370 & \textcolor{red}{ANOMALY} \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Phân tích:}
\begin{itemize}
    \item Normal sample có error (0.062347) thấp hơn threshold đáng kể (79.5\% của threshold)
    \item Anomaly samples có error cao gấp 1.59-6.54 lần threshold
    \item Sensor error (0.512309) có error cao nhất, gấp 6.54 lần threshold - dễ phát hiện nhất
    \item Negative values (0.124484) có error gấp 1.59 lần threshold
    \item High vibration (0.215080) có error gấp 2.74 lần threshold
...
FL (IID) & 0.004064 & Tốt & Trung bình (models) \\
FL (Non-IID) & 0.003787 & Tốt & Trung bình (models) \\
