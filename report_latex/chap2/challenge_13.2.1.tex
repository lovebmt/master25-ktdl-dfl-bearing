...
mạng được định nghĩa trước (vòng tròn, lưới, gossip, hoặc dựa trên blockchain).  
...
Nhờ vậy mô hình vẫn hội tụ ngay cả khi cấu trúc mạng thay đổi liên tục.
    \item \textbf{Thuật toán Gossip giúp chống lỗi tự nhiên}
    \begin{itemize}
...
Đây là cơ chế đặc biệt phù hợp với mạng IoT dạng lưới và IoT công nghiệp.
    \item \textbf{Cơ chế bỏ qua hoặc giảm ảnh hưởng của các node chậm}
    \begin{itemize}
    \item Các node không cần chờ nhau.
    \item Node chậm sẽ không làm ngưng toàn bộ tiến trình.
    \item Khi nhận mô hình quá cũ, node nhanh có thể bỏ qua cập nhật lạc hậu.
\end{itemize}
Điều này giúp mô hình hội tụ nhanh hơn và ổn định hơn so với FL tập trung.
    \item \textbf{Cơ chế xử lý nhiễu và mất gói tin}
    \begin{itemize}
    \item Truyền nhiều bản sao mô hình trong phát sóng gossip.
    \item Truyền mô hình nhỏ gọn hơn (nén gradient).
    \item Lọc hoặc làm mượt mô hình bị nhiễu thông qua lấy trung bình.
\end{itemize}
...
Điều này giải quyết hai rủi ro lớn:
    \begin{itemize}
    \item Không có điểm lỗi đơn: Một node chết thì chỉ mất một nguồn dữ liệu, quá trình học vẫn tiếp tục.
    \item Không tắc nghẽn tại server: Không có node đóng vai trò điểm nghẽn của toàn bộ mạng.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Không có điểm lỗi đơn}: mạng hoạt động ngay cả khi nhiều node ngừng hoạt động.
    \item \textbf{Độ trễ thấp}: giao tiếp chủ yếu diễn ra ở mạng cục bộ, thay vì gửi lên cloud.
    \item \textbf{Khả năng mở rộng cao}: số lượng node lớn không tạo ra nghẽn mạng như mô hình FL truyền thống.
    \item \textbf{Tính riêng tư mạnh hơn}: không có trung tâm lưu trữ các bản cập nhật mô hình.
    \item \textbf{Phù hợp môi trường edge}: hoạt động tốt khi kết nối không ổn định hoặc gián đoạn.
    \item \textbf{Tận dụng cụm tự nhiên của IoT}: các cảm biến trong cùng khu vực có thể liên kết với nhau hiệu quả.
\end{itemize}

Những đặc điểm này khiến DFL trở thành kiến trúc lý tưởng cho các ứng dụng như 
nhà máy thông minh, nhà thông minh, xe tự hành và mạng cảm biến phân tán.
...
\begin{itemize}
    \item Tấn công Byzantine
    \item Đầu độc mô hình
    \item Chèn backdoor
    \item Tấn công suy luận
\end{itemize}
