...
\begin{itemize}
    ...
    \item Mạng IoT khó tái tạo: Các yếu tố như mất gói, độ trễ cao, hoặc thay đổi cấu trúc mạng rất khó mô phỏng chính xác trên thiết bị thật.
\end{itemize}
...
\begin{itemize}
    ...
    \item Khả năng tái lập dễ dàng.
    \item Hỗ trợ thử nghiệm nhiều cấu trúc mạng (lưới, vòng tròn, gossip ngẫu nhiên) mà không cần thay đổi phần cứng.
\end{itemize}
...
\begin{itemize}
    ...
    \item Không có server trung tâm - hoàn toàn phi tập trung.
    \item Mỗi peer thực hiện aggregation cục bộ với mô hình nhận được từ peer trước đó.
    \end{itemize}

    \item \textbf{Triển khai bằng Python}
    \begin{itemize}
    \item Sử dụng PyTorch cho việc huấn luyện và quản lý mô hình.
    \item Mỗi peer được mô phỏng như một tiến trình độc lập.
    \item Giao tiếp giữa các peer được mô phỏng thông qua trao đổi trọng số mô hình.
    \item Hỗ trợ cả phân phối dữ liệu IID (cân bằng) và Non-IID (không cân bằng).
    \item Lấy cảm hứng từ Flower framework \cite{beutel2020} nhưng triển khai thuần túy DFL P2P.
    \end{itemize}
    ...
    \item Giảm điểm lỗi đơn.
    \item Phù hợp cho môi trường IoT với tài nguyên hạn chế.
    \item Dễ dàng mở rộng và tùy chỉnh cấu trúc mạng.
    \end{itemize}
\end{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item Mỗi cảm biến ghi nhận dữ liệu theo thời gian, tạo ra tập dữ liệu riêng (IID hoặc Non-IID).        
